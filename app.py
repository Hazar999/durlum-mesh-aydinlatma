...
def draw_comparison_chart(no_mesh: float, current: float, target: float) -> str:
    ...
    for label, value, css in bars:
        ...
        bar_html += f"""
        <div class='bar-wrap'>
            <div class='bar-value'>{int(round(value))}</div>
            <div class='bar {css}' style='height:{h:.1f}px;'></div>
    ...
    <div class="compare-box simple-chart">
        ...
        <div class="target-line" style="top:{target_top:.1f}px;"></div>
        <div class="target-pill" style="top:{target_top:.1f}px;">Hedef {int(round(target))}</div>
        <div class="chart-area">{bar_html}</div>
    </div>
    """
    ...
    <div class="app-shell">
      <div class="app-topbar">
        <div class="brand-wrap">
          <div class="logo-box"><img src="data:image/jpeg;base64,{LOGO_B64}" alt="durlum" /></div>
          <div>
...
with tab_light:
    ...
    with left:
        ...
        transmission_mode = st.radio("Meshed ışık geçirgenliği kaynağı", ["Katalog OA değerini geçici tahmin olarak kullan", "Ölçülmüş geçirgenlik değerini kullan"])
        ...
        st.text_input("Yön / Renk / Mesafe düzeltme katsayısı", value=f"{correction:.2f}" if transmission_mode.startswith("Katalog") else f"{transmission:.2f}", disabled=True)
        ...
        utilization = st.slider("Armatür verimliliği (UF)", 0.10, 1.00, 0.60, 0.01)
