...
def build_durlum_fixture_catalog() -> pd.DataFrame:
    """Build a structured fixture catalogue from the uploaded POINT, LINE and PLANE catalogues.

    Only values explicitly printed in the catalogues are entered directly. For linear products,
    total flux and power are calculated from the catalogue's lm/m and W/m values. For LUMEO-C,
    power is derived from the printed efficacy because a direct wattage is not listed.
    """
    ...
    def add_direct(
        ...
        *,
        optics: str = "",
        beam_half_angle_deg: float | None = None,
        cri: str = ">80",
        ip: str = "IP20",
        dimming: str = "DALI",
        dimensions: str = "",
        power_source: str = "Katalogda doğrudan",
        note: str = "",
    ) -> None:
        efficacy = lumens / power_w if power_w > 0 else math.nan
        rows.append({
            "geometry": geometry,
            "product": product,
            "variant": variant,
            ...
            "lumens": float(lumens),
            "efficacy_lm_w": float(efficacy),
            "length_mm": math.nan,
            "lumens_per_m": math.nan,
            "power_w_per_m": math.nan,
            "optics": optics,
            ...
            "catalogue_page": int(page),
            "catalogue": f"durlum lighting-{geometry.lower()}",
    ...
    def add_linear(
        ...
        page: int,
        *,
        optics: str = "PMMA diffuse",
        cri: str = ">80",
        ip: str = "IP20",
        note: str = "",
    ) -> None:
        length_m = length_mm / 1000.0
        power_w = power_w_per_m * length_m
        lumens = lumens_per_m * length_m
        rows.append({
            "geometry": "LINE",
            "product": product,
            "variant": f"{length_mm} mm · {cct_k} K · {optics}",
            "cct_k": int(cct_k),
            "power_w": float(power_w),
            "lumens": float(lumens),
            "efficacy_lm_w": float(lumens / power_w) if power_w > 0 else math.nan,
            "length_mm": int(length_mm),
            "lumens_per_m": float(lumens_per_m),
            "power_w_per_m": float(power_w_per_m),
            "optics": optics,
            "beam_half_angle_deg": math.nan,
            "cri": cri,
            "ip": ip,
            "dimming": "DALI",
            "dimensions": f"L={length_mm} mm",
            "catalogue_page": int(page),
            "catalogue": "durlum lighting-line",
            "power_source": "Katalog W/m × uzunluk",
            "note": note,
        })

    # POINT catalogue – direct fixture flux and power.
    add_direct("POINT", "PUNTEO-S20", "10×1 W · 2×30°", 3000, 10, 810, 15, beam_half_angle_deg=30, dimensions="Modül 500; POLYLAM min. 600")
    add_direct("POINT", "PUNTEO-S20", "10×1 W · 2×30°", 4000, 10, 1060, 15, beam_half_angle_deg=30, dimensions="Modül 500; POLYLAM min. 600")
    add_direct("POINT", "PUNTEO-S25", "Tunable White · 5×4 W · 2×45°", 2700, 20, 1020, 17, beam_half_angle_deg=45, dimensions="Modül 500; POLYLAM min. 600", note="Katalogda 2700 K ve 5000 K uç değerleri ayrı ışık akılarıyla verilir.")
    add_direct("POINT", "PUNTEO-S25", "Tunable White · 5×4 W · 2×45°", 5000, 20, 1210, 17, beam_half_angle_deg=45, dimensions="Modül 500; POLYLAM min. 600", note="Katalogda 2700 K ve 5000 K uç değerleri ayrı ışık akılarıyla verilir.")
    add_direct("POINT", "PUNTEO-S25 3X3", "Tunable White · 9×4 W · 2×45°", 2700, 36, 1830, 19, beam_half_angle_deg=45, dimensions="275×275×57 mm")
    add_direct("POINT", "PUNTEO-S25 3X3", "Tunable White · 9×4 W · 2×45°", 5000, 36, 2180, 19, beam_half_angle_deg=45, dimensions="275×275×57 mm")
    add_direct("POINT", "PUNTEO-N 5X5", "40 W · 2×30°", 4000, 40, 3625, 23, beam_half_angle_deg=30, dimensions="275×275 mm; tavan kesimi 257×257 mm", note="Katalog sayfasında armatür 40 W, sürücü satırında 18 W yazıyor. Güç hesabında armatür satırındaki 40 W kullanıldı; sipariş öncesi teknik föyle doğrulayın.")
    add_direct("POINT", "PUNTEO-Q50", "15 W · 2×55°", 3000, 15, 1100, 27, beam_half_angle_deg=55, ip="IP65", dimensions="50×50 mm; boşluk ≥60 mm")
    add_direct("POINT", "PUNTEO-Q50", "15 W · 2×55°", 4000, 15, 1440, 27, beam_half_angle_deg=55, ip="IP65", dimensions="50×50 mm; boşluk ≥60 mm")
    add_direct("POINT", "PUNTEO-G70", "13 W · medium beam", 3000, 13, 900, 31, dimensions="Ø70 mm; boşluk ≥60 mm")
    add_direct("POINT", "PUNTEO-G70", "13 W · medium beam", 4000, 13, 1055, 31, dimensions="Ø70 mm; boşluk ≥60 mm")
    for product in ["PUNTEO-J85", "PUNTEO-J85T"]:
        page = 35 if product == "PUNTEO-J85" else 37
        dims = "Ø85 mm; kesim Ø74 mm" if product == "PUNTEO-J85" else "Ø125 mm; kesim Ø110 mm"
        for beam in [20, 30, 40]:
            for power, cct, lm in [(18, 3000, 2286), (18, 4000, 2390), (25, 3000, 3012), (25, 4000, 3149)]:
                add_direct("POINT", product, f"{power} W · 2×{beam}°", cct, power, lm, page, beam_half_angle_deg=beam, dimensions=dims)
    add_direct("POINT", "PUNTEO-P155", "Tunable White endpoint · 36 W · 2×35°", 2700, 36, 1170, 41, beam_half_angle_deg=35, cri=">85", dimensions="Ø155 mm; kesim Ø145 mm")
    add_direct("POINT", "PUNTEO-P155", "Tunable White endpoint · 36 W · 2×35°", 5000, 36, 1470, 41, beam_half_angle_deg=35, cri=">85", dimensions="Ø155 mm; kesim Ø145 mm")
    add_direct("POINT", "PUNTEO-P155", "Single colour · 36 W · 2×35°", 3000, 36, 2550, 41, beam_half_angle_deg=35, cri=">85", dimensions="Ø155 mm; kesim Ø145 mm")
    add_direct("POINT", "PUNTEO-P155", "Single colour · 36 W · 2×35°", 4000, 36, 2910, 41, beam_half_angle_deg=35, cri=">85", dimensions="Ø155 mm; kesim Ø145 mm")
    add_direct("POINT", "PUNTEO-P245", "56 W · 2×45°", 4000, 56, 5670, 45, beam_half_angle_deg=45, dimensions="Ø245 mm")
    add_direct("POINT", "PUNTEO-P64", "3 W · 2×10°", 3000, 3, 140, 49, beam_half_angle_deg=10, ip="IP64", dimensions="Kesim Ø48 mm")
    add_direct("POINT", "PUNTEO-P64", "3 W · 2×10°", 4000, 3, 160, 49, beam_half_angle_deg=10, ip="IP64", dimensions="Kesim Ø48 mm")
    for beam in [10, 20]:
        add_direct("POINT", "PUNTEO-Y255 HYBRID", f"56 W · 2×{beam}°", 4000, 56, 5600, 53, beam_half_angle_deg=beam, ip="IP65", dimensions="Ø255 mm")
        add_direct("POINT", "PUNTEO-Y300 HYBRID", f"112 W · 2×{beam}°", 4000, 112, 9256, 55, beam_half_angle_deg=beam, ip="IP65", dimensions="Ø300 mm")

    # LINE catalogue – flux and power per metre converted to each listed length.
    for length in [980, 1980, 2980]:
        add_linear("TANGENTA-Q24", length, 3000, 16.8, 1820, 15, cri=">90")
        add_linear("TANGENTA-Q24", length, 4000, 16.8, 1840, 15, cri=">90")
    for length in [854, 1134, 1414, 2254]:
        add_linear("OMEGA 40 SYSTEM", length, 3000, 22.9, 3796, 19)
        add_linear("OMEGA 40 SYSTEM", length, 4000, 22.9, 3968, 19)
    for length in [574, 854, 1134, 1414, 2254]:
        add_linear("OMEGA 63 SYSTEM", length, 3000, 27.8, 4454, 25, optics="Simetrik PMMA diffuse")
        add_linear("OMEGA 63 SYSTEM", length, 4000, 27.8, 4654, 25, optics="Simetrik PMMA diffuse")
    for length in [980, 1980, 2980]:
        add_linear("OMEGA 63 SYSTEM", length, 3000, 22.9, 2190, 25, optics="Wallwasher")
        add_linear("OMEGA 63 SYSTEM", length, 4000, 22.9, 2500, 25, optics="Wallwasher")
    for length in [574, 854, 1134, 1414, 2054]:
        add_linear("OMEGA 100 / 100E SYSTEM", length, 3000, 28.4, 4626, 31)
        add_linear("OMEGA 100 / 100E SYSTEM", length, 4000, 28.4, 4832, 31)
    for product, page, p_m, lm_3k, lm_4k, lengths in [
        ("TANGENTA-C40", 37, 22.9, 3796, 3968, [854, 1134, 1414]),
        ("TANGENTA-C63", 39, 27.8, 4454, 4654, [854, 1134, 1414]),
        ("TANGENTA-C100", 41, 28.4, 4625, 4830, [854, 1134, 1414]),
        ("SECURA-C100", 45, 22.9, 3741, 3910, [854, 1134, 1414]),
    ]:
        for length in lengths:
            add_linear(product, length, 3000, p_m, lm_3k, page, optics="PMMA diffuse / microprismatic seçenekleri")
            add_linear(product, length, 4000, p_m, lm_4k, page, optics="PMMA diffuse / microprismatic seçenekleri")
    add_linear("TANGENTA-Y100", 1562, 4000, 66.7, 8650, 49, ip="IP54", optics="Diffuse")

    # PLANE catalogue – direct fixture data.
    for product, page, sizes in [
        ("LUMEO-R", 11, [("Ø594 mm", 72, 10120, 10600), ("Ø894 mm", 152, 19200, 20120), ("Ø1194 mm", 198, 28480, 29840)]),
        ("LUMEO-X", 19, [("Ø670 mm", 72, 10120, 10600), ("Ø990 mm", 152, 19200, 20120), ("Ø1300 mm", 198, 28480, 29840)]),
    ]:
        for dims, power, lm_3k, lm_4k in sizes:
            optics_list = ["DUROSATIN"] if product == "LUMEO-X" or "1194" in dims else ["DUROSATIN", "MICROPRISMATIC"]
            for optics in optics_list:
                add_direct("PLANE", product, f"{dims} · {optics}", 3000, power, lm_3k, page, optics=optics, dimensions=dims)
                add_direct("PLANE", product, f"{dims} · {optics}", 4000, power, lm_4k, page, optics=optics, dimensions=dims)
    for dims, power, lm_3k, lm_4k, optics_list in [
        ("600×600 mm", 64, 6840, 7080, ["DUROSATIN", "MICROPRISMATIC"]),
        ("900×900 mm", 133, 15390, 15930, ["DUROSATIN", "MICROPRISMATIC"]),
        ("1200×1200 mm", 240, 27360, 28320, ["DUROSATIN"]),
    ]:
        for optics in optics_list:
            add_direct("PLANE", "LUMEO-Q", f"{dims} · {optics}", 3000, power, lm_3k, 23, optics=optics, dimensions=dims)
            add_direct("PLANE", "LUMEO-Q", f"{dims} · {optics}", 4000, power, lm_4k, 23, optics=optics, dimensions=dims)
    for cct, lm in [(3000, 8550), (4000, 8850)]:
        for optics in ["DUROSATIN", "MICROPRISMATIC"]:
            derived_power = lm / 80.0
            add_direct(
                "PLANE", "LUMEO-C", f"1250×250×81 mm · {optics}", cct,
                derived_power, lm, 33, optics=optics, dimensions="1250×250×81 mm",
                power_source="80 lm/W katalog verisinden türetildi",
                note="Katalog sayfasında doğrudan W değeri verilmediği için güç, basılı 80 lm/W verimden hesaplandı.",
            )

    df = pd.DataFrame(rows)
    df["selection_label"] = (
        df["product"].astype(str) + " · " + df["variant"].astype(str) + " · " +
        df["cct_k"].astype(int).astype(str) + " K · " + df["lumens"].round(0).astype(int).astype(str) + " lm"
    )
    return df.sort_values(["geometry", "product", "cct_k", "lumens"]).reset_index(drop=True)


def room_index(room_length: float, room_width: float, mounting_height_above_workplane: float) -> float:
    denominator = mounting_height_above_workplane * (room_length + room_width)
    return (room_length * room_width / denominator) if denominator > 0 else 0.0


def detailed_lighting_values(
    room_area_m2: float,
    fixture_count: int,
    ...
    transmission: float,
    target_artificial_lux: float,
    daylight_lux: float = 0.0,
) -> dict[str, float]:
    effective_flux_per_fixture = lumens_per_fixture * utilization_factor * maintenance_factor * transmission
    artificial_lux = fixture_count * effective_flux_per_fixture / room_area_m2 if room_area_m2 > 0 else 0.0
    unobstructed_lux = (
        fixture_count * lumens_per_fixture * utilization_factor * maintenance_factor / room_area_m2
        if room_area_m2 > 0 else 0.0
    )
    required = (
        math.ceil(target_artificial_lux * room_area_m2 / effective_flux_per_fixture)
        if effective_flux_per_fixture > 0 and target_artificial_lux > 0 else 0
    )
    return {
        "artificial_lux": artificial_lux,
        "combined_lux": artificial_lux + daylight_lux,
        "unobstructed_lux": unobstructed_lux,
        "effective_flux_per_fixture": effective_flux_per_fixture,
        "required_count": required,
        "loss_pct": max(0.0, (1.0 - transmission) * 100.0),
    }
...
def apply_lighting_room_preset() -> None:
    selected_room = st.session_state.get("lc_room_type", "Ofis / çalışma alanı")
    if selected_room != "Özel / elle gir":
        st.session_state["lc_target_lux"] = ROOM_TARGETS[selected_room]


def render_lighting_calculation_module(fixture_catalog_df: pd.DataFrame, meshes: pd.DataFrame) -> None:
    st.markdown(
        "<div class='section-title'>Durlum Aydınlatma Hesabı</div>"
        "<div class='section-subtitle'>POINT, LINE ve PLANE katalog ürünlerini gerçek katalog lümenleriyle seçin; lümen yöntemi, mesh etkisi, günışığı katkısı, güç ve yaklaşık yerleşimi birlikte değerlendirin.</div>",
        unsafe_allow_html=True,
    )
    tab_calc, tab_daylight, tab_catalog = st.tabs(["✦ Aydınlatma hesabı", "☀ Günışığı / DAYTEC", "⌘ Ürün kataloğu"])

    with tab_calc:
        input_col, result_col = st.columns([1.02, 1.9], gap="large")
        with input_col:
            with st.container(border=True):
                st.markdown("<div class='input-group-title'>Mekân ve hedef</div>", unsafe_allow_html=True)
                room_type = st.selectbox(
                    "Mekân türü",
                    list(ROOM_TARGETS.keys()),
                    key="lc_room_type",
                    on_change=apply_lighting_room_preset,
                )
                room_length = st.number_input("Mekân uzunluğu (m)", 1.0, 500.0, 12.0, 0.5, key="lc_length")
                room_width = st.number_input("Mekân genişliği (m)", 1.0, 500.0, 10.0, 0.5, key="lc_width")
                room_height = st.number_input("Net mekân yüksekliği (m)", 1.5, 30.0, 3.2, 0.1, key="lc_height")
                workplane_height = st.number_input("Hesap düzlemi yüksekliği (m)", 0.0, 2.0, 0.80, 0.05, key="lc_workplane")
                suspension = st.number_input("Armatürün tavandan sarkma mesafesi (m)", 0.0, 10.0, 0.0, 0.05, key="lc_suspension")
                target_lux = st.number_input("Hedef ortalama aydınlık (lux)", 1.0, 5000.0, step=25.0, key="lc_target_lux")

                st.markdown("<div class='input-group-title'>Durlum armatür seçimi</div>", unsafe_allow_html=True)
                geometry = st.selectbox("Işık geometrisi", ["POINT", "LINE", "PLANE"], key="lc_geometry")
                geometry_df = fixture_catalog_df.loc[fixture_catalog_df["geometry"] == geometry]
                product = st.selectbox("Ürün", geometry_df["product"].drop_duplicates().tolist(), key=f"lc_product_{geometry}")
                product_df = geometry_df.loc[geometry_df["product"] == product]
                selection_label = st.selectbox(
                    "Varyant",
                    product_df["selection_label"].tolist(),
                    key=f"lc_variant_{geometry}_{product}",
                )
                fixture = product_df.loc[product_df["selection_label"] == selection_label].iloc[0]
                existing_count = st.number_input("Mevcut / denenecek armatür adedi", 0, 10000, 0, 1, key="lc_count")

                st.markdown("<div class='input-group-title'>Hesap katsayıları</div>", unsafe_allow_html=True)
                uf_source = st.radio(
                    "Kullanım faktörü (UF) kaynağı",
                    ["IES/LDT veya DIALux/Relux sonucundan biliniyor", "Ön tasarım varsayımı"],
                    index=1,
                    key="lc_uf_source",
                )
                utilization = st.slider("Kullanım faktörü (UF)", 0.10, 1.00, 0.60, 0.01, key="lc_uf")
                maintenance = st.slider("Bakım faktörü (MF)", 0.10, 1.00, 0.80, 0.01, key="lc_mf")

                st.markdown("<div class='input-group-title'>Tavan / mesh etkisi</div>", unsafe_allow_html=True)
                obstruction_mode = st.radio(
                    "Armatür konumu ve geçirgenlik",
                    [
                        "Armatür tavan altında / entegre – ek kayıp yok",
                        "Mesh üstünde – ölçülmüş optik geçirgenlik",
                        "Mesh üstünde – katalog OA ile hızlı ön hesap",
                    ],
                    key="lc_obstruction",
                )
                transmission = 1.0
                transmission_note = "Ek tavan geçirgenlik kaybı uygulanmadı."
                if obstruction_mode.startswith("Mesh üstünde – ölçülmüş"):
                    measured_t = st.number_input("Ölçülmüş optik geçirgenlik (%)", 1.0, 100.0, 75.0, 0.5, key="lc_measured_t")
                    transmission = measured_t / 100.0
                    transmission_note = "Kullanıcı tarafından girilen ölçülmüş optik geçirgenlik kullanıldı."
                elif obstruction_mode.startswith("Mesh üstünde – katalog"):
                    mesh_code = st.selectbox("Mesh kodu", meshes["mesh_code"].tolist(), key="lc_mesh")
                    mesh_row = meshes.loc[meshes["mesh_code"] == mesh_code].iloc[0]
                    oa_correction = st.slider("OA → optik geçirgenlik düzeltme katsayısı", 0.50, 1.00, 0.90, 0.01, key="lc_oa_correction")
                    transmission = min(1.0, float(mesh_row["open_area_pct"]) / 100.0 * oa_correction)
                    transmission_note = "OA gerçek optik geçirgenlik değildir; bu sonuç yalnızca hızlı ön hesaptır."

                st.markdown("<div class='input-group-title'>Günışığı senaryosu</div>", unsafe_allow_html=True)
                use_daylight = st.checkbox("DAYTEC günışığı katkısını senaryoya ekle", value=False, key="lc_use_daylight")
                daylight_lux = 0.0
                daylight_factor = 0.0
                exterior_lux = 10000.0
                daylight_basis = "Yapay aydınlatma tek başına hedefi karşılasın"
                if use_daylight:
                    daylight_factor = st.number_input("Ortalama günışığı faktörü DF (%)", 0.0, 20.0, 2.0, 0.1, key="lc_df")
                    exterior_lux = st.number_input("Dış yatay aydınlık – kapalı gökyüzü (lux)", 100.0, 100000.0, 10000.0, 500.0, key="lc_exterior")
                    daylight_lux = exterior_lux * daylight_factor / 100.0
                    daylight_basis = st.radio(
                        "Günışığının hesapta kullanımı",
                        [
                            "Yapay aydınlatma tek başına hedefi karşılasın",
                            "Günışığı kontrollü işletme: yapay ışık kalan ihtiyacı tamamlasın",
                        ],
                        key="lc_daylight_basis",
                    )

        room_area = room_length * room_width
        mounting_height = room_height - suspension - workplane_height
        k_room = room_index(room_length, room_width, mounting_height)
        artificial_target = target_lux
        if use_daylight and daylight_basis.startswith("Günışığı kontrollü"):
            artificial_target = max(0.0, target_lux - daylight_lux)

        result = detailed_lighting_values(
            room_area,
            int(existing_count),
            float(fixture["lumens"]),
            float(utilization),
            float(maintenance),
            float(transmission),
            float(artificial_target),
            float(daylight_lux if use_daylight else 0.0),
        )
        required_count = int(result["required_count"])
        ...
        recommended_result = detailed_lighting_values(
            room_area,
            recommended_count,
            float(fixture["lumens"]),
            float(utilization),
        ...
        power_w = float(fixture["power_w"])
        existing_power_kw = int(existing_count) * power_w / 1000.0
        recommended_power_kw = recommended_count * power_w / 1000.0
        recommended_w_m2 = recommended_count * power_w / room_area if room_area > 0 else 0.0
        ...
        with result_col:
            st.markdown(
                ...
                <div class='hero-card'>
                  <div class='hero-content'>
                    <h2>{fixture['product']} · {fixture['geometry']}</h2>
                    <p>{fixture['variant']} · {int(fixture['cct_k'])} K · {fixture['lumens']:.0f} lm · {fixture['power_w']:.1f} W. Hesap, katalog ışık akısını lümen yöntemiyle kullanır; kesin düzgünlük ve kamaşma için IES/LDT fotometrisi gerekir.</p>
                  </div>
                </div>
                """,
                unsafe_allow_html=True,
            ...
            product_cards = st.columns(4)
            product_values = [
                ("Armatür ışık akısı", f"{fixture['lumens']:.0f} lm", "Katalog varyantı"),
                ("Armatür gücü", f"{fixture['power_w']:.1f} W", str(fixture['power_source'])),
                ("Hesaplanan etkinlik", f"{fixture['efficacy_lm_w']:.1f} lm/W", "lm ÷ W"),
                ("CRI / IP", f"{fixture['cri']} · {fixture['ip']}", "Katalog"),
            ]
            for col, (title, value, sub) in zip(product_cards, product_values):
                with col:
                    st.markdown(metric_card(title, value, sub), unsafe_allow_html=True)

            st.markdown("<div class='section-title' style='margin-top:14px;'>Hesap sonucu</div>", unsafe_allow_html=True)
            result_cards = st.columns(4)
            combined_existing = result["combined_lux"] if use_daylight else result["artificial_lux"]
            result_values = [
                ("Mevcut yapay aydınlık", f"{result['artificial_lux']:.0f} lux", f"{int(existing_count)} armatür"),
                ("Günışığı senaryosu", f"{daylight_lux:.0f} lux" if use_daylight else "Kapalı", f"DF %{daylight_factor:.1f}" if use_daylight else "Hesaba katılmadı"),
                ("Birleşik senaryo", f"{combined_existing:.0f} lux", f"Hedef {target_lux:.0f} lux"),
                ("Gerekli armatür", f"{required_count} adet", f"Grid önerisi {recommended_count} adet"),
            ]
            for col, (title, value, sub) in zip(result_cards, result_values):
                with col:
                    st.markdown(metric_card(title, value, sub), unsafe_allow_html=True)

            detail_cards = st.columns(4)
            detail_values = [
                ("Oda indeksi K", f"{k_room:.2f}", f"Hm={mounting_height:.2f} m"),
                ("Etkin geçirgenlik", f"%{transmission*100:.1f}", transmission_note),
                ("Önerilen kurulu güç", f"{recommended_power_kw:.2f} kW", f"{recommended_w_m2:.2f} W/m²"),
                ("Mevcut kurulu güç", f"{existing_power_kw:.2f} kW", f"{existing_count} adet"),
            ]
            for col, (title, value, sub) in zip(detail_cards, detail_values):
                with col:
                    st.markdown(metric_card(title, value, sub), unsafe_allow_html=True)

            st.markdown(
                f"<div class='info-callout'><b>Kullanılan formül:</b> Ē = N × Φ × UF × MF × T / A. "
                f"Bu projede bir armatürün hesap düzlemine ulaşan etkin akısı yaklaşık <b>{result['effective_flux_per_fixture']:.0f} lm</b>. "
                f"Yapay ışık tasarım hedefi <b>{artificial_target:.0f} lux</b>.</div>",
                unsafe_allow_html=True,
            )

            if mounting_height <= 0:
                st.error("Hesap düzlemi, armatür yüksekliğinin üzerinde kaldı. Mekân yüksekliği, sarkma ve hesap düzlemi değerlerini düzeltin.")
            if uf_source.startswith("Ön tasarım"):
                st.warning("UF=0,60 yalnızca ön tasarım varsayımıdır. Kesin hesap için seçili armatürün IES/LDT dosyasından DIALux/Relux kullanım faktörü veya doğrudan fotometrik sonuç kullanılmalıdır.")
            if obstruction_mode.startswith("Mesh üstünde – katalog"):
                st.warning("Mesh açık alanı (OA), optik ışık geçirgenliğiyle aynı değildir. Renk, yön, kalınlık, armatür-mesh mesafesi ve ışık dağılımı sonucu değiştirebilir.")
            if str(fixture["power_source"]).startswith("80 lm/W"):
                st.warning("LUMEO-C gücü katalogda doğrudan yazmadığı için katalog etkinliğinden türetildi. Elektrik projesinde ürün koduna ait güncel teknik föyle doğrulayın.")
            if str(fixture["note"]).strip():
                st.info(str(fixture["note"]))

            st.markdown("<div class='section-title' style='margin-top:14px;'>Yaklaşık yerleşim</div>", unsafe_allow_html=True)
            if primary_grid:
                max_spacing = max(float(primary_grid["length_spacing"]), float(primary_grid["width_spacing"]))
                spacing_ratio = max_spacing / mounting_height if mounting_height > 0 else math.nan
                coverage_text = ""
                beam_half = fixture["beam_half_angle_deg"]
                if pd.notna(beam_half) and mounting_height > 0:
                    coverage_diameter = 2.0 * mounting_height * math.tan(math.radians(float(beam_half)))
                    coverage_text = f" · geometrik ışık konisi çapı yaklaşık {coverage_diameter:.2f} m"
                st.markdown(
                    f"""
                    <div class='layout-card'>
                      <div style='display:grid;grid-template-columns:1fr auto;gap:18px;align-items:center;'>
                        <div>
                          <b>{int(primary_grid['rows'])} × {int(primary_grid['columns'])} = {int(primary_grid['total'])} adet</b><br>
                          Boy yönü merkez aralığı: {primary_grid['length_spacing']:.2f} m · en yönü merkez aralığı: {primary_grid['width_spacing']:.2f} m<br>
                          Maksimum merkez aralığı / Hm: {spacing_ratio:.2f}{coverage_text}<br>
                          Önerilen düzende yapay aydınlık: {recommended_result['artificial_lux']:.0f} lux · birleşik senaryo: {recommended_result['combined_lux']:.0f} lux
                        </div>
                        <div>{grid_preview(int(primary_grid['rows']), int(primary_grid['columns']))}</div>
                      </div>
                    </div>
                    """,
                    unsafe_allow_html=True,
                ...
                st.info("Gerekli armatür adedi 0 olduğu için yerleşim gridi oluşturulmadı.")

            st.markdown(
                "<div class='callout'><b>Hesabın sınırı:</b> Lümen yöntemi ortalama yatay aydınlığı ön boyutlandırır. "
                "Uniformite, UGR/kamaşma, düşey ve silindirik aydınlık, acil aydınlatma, gölgelenme ve gerçek tavan geometrisi için IES/LDT tabanlı fotometrik simülasyon zorunludur.</div>",
                unsafe_allow_html=True,
            )

    with tab_daylight:
        st.markdown("<div class='section-title'>DAYTEC günışığı ön değerlendirmesi</div>", unsafe_allow_html=True)
        st.markdown(
            """
            <div class='product-summary'>
            <b>Günışığı faktörü (DF)</b>, kapalı gökyüzünde dışarıdaki kullanılabilir günışığının iç mekânda bulunan yüzdesidir.
            DAYTEC kataloğunda ortalama DF ≥ %2 genellikle yeterli, DF ≥ %5 ise gerçekten aydınlık / çok iyi olarak tanımlanır.
            Katalog, kapalı gökyüzü için yaklaşık 10.000 lux dış aydınlığı örnekler. Bu nedenle hızlı senaryoda E<sub>daylight</sub> = E<sub>outside</sub> × DF / 100 kullanılır.
            </div>
            """,
            unsafe_allow_html=True,
        )
        ...
        with d1:
            df_demo = st.number_input("DF (%)", 0.0, 20.0, 2.0, 0.1, key="daytec_demo_df")
        with d2:
            outside_demo = st.number_input("Dış aydınlık (lux)", 100.0, 100000.0, 10000.0, 500.0, key="daytec_demo_out")
        day_demo = outside_demo * df_demo / 100.0
        with d3:
            st.markdown(metric_card("Tahmini iç günışığı", f"{day_demo:.0f} lux", "Kapalı gökyüzü senaryosu"), unsafe_allow_html=True)
        if df_demo >= 5:
            st.success("DAYTEC katalog sınıflamasına göre DF ≥ %5: çok iyi / gerçekten aydınlık.")
        elif df_demo >= 2:
            st.info("DAYTEC katalog sınıflamasına göre DF ≥ %2: genellikle yeterli günışığı.")
        else:
            st.warning("DF %2'nin altında: günışığı katkısı sınırlı olabilir; geometri ve günışığı sistemi simülasyonla geliştirilmelidir.")
        st.warning("Bu basit DF hesabı PAREA veya başka bir günışığı simülasyonunun yerine geçmez. İklim, yön, saat, gökyüzü modeli, cam, gölgeleme, g-değeri, kamaşma ve dağılım ayrıca değerlendirilmelidir.")

    with tab_catalog:
        display_cols = [
            "geometry", "product", "variant", "cct_k", "lumens", "power_w", "efficacy_lm_w",
            "lumens_per_m", "power_w_per_m", "cri", "ip", "dimming", "dimensions",
            "catalogue_page", "power_source", "note",
        ]
        st.dataframe(
            fixture_catalog_df[display_cols].rename(columns={
                "geometry": "Geometri", "product": "Ürün", "variant": "Varyant", "cct_k": "CCT (K)",
                "lumens": "Işık akısı (lm)", "power_w": "Güç (W)", "efficacy_lm_w": "Hesaplanan lm/W",
                "lumens_per_m": "lm/m", "power_w_per_m": "W/m", "cri": "CRI", "ip": "IP",
                "dimming": "Kontrol", "dimensions": "Ölçü", "catalogue_page": "Katalog sayfası",
                "power_source": "Güç kaynağı", "note": "Not",
            }),
            use_container_width=True,
            hide_index=True,
            column_config={
                "Işık akısı (lm)": st.column_config.NumberColumn(format="%.0f"),
                "Güç (W)": st.column_config.NumberColumn(format="%.1f"),
                "Hesaplanan lm/W": st.column_config.NumberColumn(format="%.1f"),
                "lm/m": st.column_config.NumberColumn(format="%.0f"),
                "W/m": st.column_config.NumberColumn(format="%.1f"),
            },
        )
        st.info("LUMEO-E ve LUMEO-O katalogda proje özel şekiller olarak sunuluyor; yüklenen katalogda hesapta kullanılabilecek sabit lümen/güç tablosu bulunmadığı için otomatik armatür hesabına eklenmedi. LUMEO-Q CEILING, LUMEO-Q tekil eleman teknik verilerini kullanır.")
...
fixture_catalog_df = build_durlum_fixture_catalog()

if "lc_room_type" not in st.session_state:
    st.session_state["lc_room_type"] = "Ofis / çalışma alanı"
if "lc_target_lux" not in st.session_state:
    st.session_state["lc_target_lux"] = ROOM_TARGETS[st.session_state["lc_room_type"]]
...
if product_family == "Aydınlatma / POINT · LINE · PLANE":
    render_lighting_calculation_module(fixture_catalog_df, meshes)
    st.caption("Kaynaklar: durlum lighting POINT (04.08.2022), LINE (04.08.2022), PLANE (15.08.2025) ve DAYTEC katalogları. Sonuç lümen yöntemiyle ön boyutlandırmadır; kesin proje için IES/LDT simülasyonu gerekir.")
    st.stop()
elif product_family == "Open Cell / Raster Tavan":
...
with tab_light:
    ...
    with left:
        with st.container(border=True):
            ...
            fixture_data_mode = st.selectbox("Armatür verisi kaynağı", ["Durlum katalog ürünü", "Manuel değer gir"], key="mesh_fixture_mode")
            if fixture_data_mode == "Durlum katalog ürünü":
                mesh_geometry = st.selectbox("Işık geometrisi", ["POINT", "LINE", "PLANE"], key="mesh_fixture_geometry")
                mesh_fixture_subset = fixture_catalog_df.loc[fixture_catalog_df["geometry"] == mesh_geometry]
                mesh_fixture_product = st.selectbox("Durlum ürünü", mesh_fixture_subset["product"].drop_duplicates().tolist(), key=f"mesh_fixture_product_{mesh_geometry}")
                mesh_product_subset = mesh_fixture_subset.loc[mesh_fixture_subset["product"] == mesh_fixture_product]
                mesh_fixture_label = st.selectbox("Ürün varyantı", mesh_product_subset["selection_label"].tolist(), key=f"mesh_fixture_variant_{mesh_geometry}_{mesh_fixture_product}")
                mesh_fixture = mesh_product_subset.loc[mesh_product_subset["selection_label"] == mesh_fixture_label].iloc[0]
                lumens = float(mesh_fixture["lumens"])
                power_w = float(mesh_fixture["power_w"])
                st.caption(f"Katalog verisi: {lumens:.0f} lm · {power_w:.1f} W · CRI {mesh_fixture['cri']} · {mesh_fixture['ip']} · sayfa {int(mesh_fixture['catalogue_page'])}")
                if str(mesh_fixture["note"]).strip():
                    st.info(str(mesh_fixture["note"]))
            else:
                lumens = st.number_input("Bir armatürün ışık akısı (lm)", min_value=100.0, max_value=200000.0, value=4000.0, step=100.0, key="mesh_manual_lm")
                power_w = st.number_input("Bir armatürün gücü (W)", min_value=0.0, max_value=5000.0, value=35.0, step=1.0, key="mesh_manual_w")
            ...
            st.caption("UF, seçili ürünün IES/LDT fotometrisi ve oda yansıtma değerleriyle doğrulanmalıdır. MF bakım ve kirlenme planına bağlıdır.")
